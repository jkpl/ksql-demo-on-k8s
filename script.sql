...
WITH (kafka_topic='pageviews', value_format='DELIMITED', partitions=1, replicas=1);
...
WITH (kafka_topic='users', value_format='JSON', key = 'userid', partitions=1, replicas=1);
...
WITH (kafka_topic='pageviews_enriched_r8_r9', partitions=1, replicas=1) AS
...
CREATE TABLE pageviews_regions WITH (VALUE_FORMAT='avro') AS
SELECT gender, regionid , COUNT(*) AS numusers
FROM pageviews_enriched WINDOW TUMBLING (size 30 second)
